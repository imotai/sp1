...
/// @note The output array is of dimension (width) and contains the sum of each column of the input array.
...
__global__ void reduceKernel(F *input, F *output, size_t width, size_t height)
{
    ...
    for (size_t j = blockIdx.y * blockDim.y + threadIdx.y; j < width;
...
{
    return reinterpret_cast<void *>(reduceKernel<felt_t>);
}

extern "C" void *reduce_kernel_ext()
{
    return reinterpret_cast<void *>(reduceKernel<ext_t>);
}