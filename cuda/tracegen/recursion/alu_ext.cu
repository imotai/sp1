...
__global__ void recursion_ext_alu_generate_preprocessed_trace_kernel(
    ...
    static const size_t COLUMNS = sizeof(csl_sys::ExtAluAccessCols<T>) / sizeof(T);
    ...
    for (; i < nb_instructions; i += blockDim.x * gridDim.x) {
        ...
        size_t start = (i % csl_sys::NUM_EXT_ALU_ENTRIES_PER_ROW) * COLUMNS;
...
__global__ void recursion_ext_alu_generate_trace_kernel(
    ...
    uintptr_t nb_events) {
    static const size_t COLUMNS = sizeof(csl_sys::ExtAluValueCols<T>) / sizeof(T);
