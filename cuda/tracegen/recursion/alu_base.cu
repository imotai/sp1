...
__global__ void recursion_base_alu_generate_preprocessed_trace_kernel(
    ...
    static const size_t COLUMNS = sizeof(csl_sys::BaseAluAccessCols<T>) / sizeof(T);
    ...
    for (; i < nb_instructions; i += blockDim.x * gridDim.x) {
        ...
        size_t start = (i % csl_sys::NUM_BASE_ALU_ENTRIES_PER_ROW) * COLUMNS;
        for (size_t j = 0; j < COLUMNS; ++j) {
            trace[(i / csl_sys::NUM_BASE_ALU_ENTRIES_PER_ROW) + (j + start) * trace_height] =
                arr[j];
        }
    }
}
...
__global__ void recursion_base_alu_generate_trace_kernel(
    ...
    uintptr_t nb_events) {
    static const size_t COLUMNS = sizeof(csl_sys::BaseAluValueCols<T>) / sizeof(T);
