...
struct CircuitValues {
  ...
  public:
    static __device__ __forceinline__ CircuitValues<EF>
    load(EF* layer, size_t i, size_t parity, size_t height) {
    ...
    static __device__ __forceinline__ CircuitValues<EF>
    load(const EF* layer, size_t i, size_t parity, size_t height) {
        CircuitValues<EF> values;
        // Load the numerator and denominator values
        //  numerator[i, b, 0] = layer[0, b, i]
        //  numerator[i, b, 1] = layer[1, b, i]
    ...
    static __device__ __forceinline__ CircuitValues<EF>
    fix_last_variable(CircuitValues<EF> zeroValues, CircuitValues<EF> oneValues, EF alpha) {
        CircuitValues<EF> values;
