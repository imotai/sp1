...
__global__ void jaggedPopulate(
    ...
    for (size_t rowIdx = blockIdx.x * blockDim.x + threadIdx.x; rowIdx < height;
        ...
        for (size_t colIdx = blockIdx.y * blockDim.y + threadIdx.y; colIdx < width;
             colIdx += gridDim.y * blockDim.y) {
...
__global__ void jaggedSumAsPoly(
    ...
    for (size_t rowIdx = blockIdx.x * blockDim.x + threadIdx.x; rowIdx < halfHeight;
         rowIdx += gridDim.x * blockDim.x) {
