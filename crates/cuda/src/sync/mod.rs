...
unsafe impl<T> CudaSend for Buffer<T, CpuBackend> {
    fn change_scope(&mut self, _scope: &TaskScope) {}
}
...
unsafe impl<T: DeviceCopy> CudaSend for Buffer<T, TaskScope> {
    fn change_scope(&mut self, scope: &TaskScope) {
        unsafe {
    ...
    async fn test_cuda_send_derive_macro() {
        ...
        let (handle_1, handle_2) = tokio::join!(
            ...
            task_2.run(|t| async move {
                ...
                assert_eq!(a, 1);
                assert_eq!(b, 2);
                assert_eq!(buffer.into_host().await.unwrap().into_vec(), vec![1; 1000000]);
            }),
        );
