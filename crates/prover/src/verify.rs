...
impl<C: SP1ProverComponents> SP1Prover<C> {
    ...
    pub fn verify(
        ...
        for shard_proof in proof.0.iter() {
            let mut challenger = self.core_verifier.pcs_verifier.challenger();
    ...
    pub fn verify_compressed(
        ...
        let mut challenger = self.compress_verifier.pcs_verifier.challenger();
...
// use slop_baby_bear::BabyBear;
use sp1_core_executor::{subproof::SubproofVerifier, SP1ReduceProof};
use sp1_stark::{BabyBearPoseidon2, MachineVerifierError};
...
impl<C: SP1ProverComponents> SubproofVerifier for SP1Prover<C> {
    fn verify_deferred_proof(
        &self,
        _proof: &sp1_core_machine::reduce::SP1ReduceProof<InnerSC>,
        _vk: &sp1_stark::MachineVerifyingKey<CoreSC>,
        _vk_hash: [u32; 8],
...
pub struct ArcProver<C: SP1ProverComponents>(pub Arc<SP1Prover<C>>);

impl<C: SP1ProverComponents> SubproofVerifier for ArcProver<C> {
    fn verify_deferred_proof(
        &self,
        proof: &sp1_core_executor::SP1ReduceProof<sp1_stark::BabyBearPoseidon2>,
        vk: &sp1_stark::MachineVerifyingKey<sp1_stark::BabyBearPoseidon2>,
        vk_hash: [u32; 8],
        committed_value_digest: [u32; 8],
    ) -> Result<(), MachineVerifierError<sp1_stark::BabyBearPoseidon2>> {
        self.0.verify_deferred_proof(proof, vk, vk_hash, committed_value_digest)
    }
}
