...
use crate::{
    merkle_tree::{MerkleTree, MerkleTreeHasher},
    VirtualTensor,
};
...
mod tests {
    ...
    use csl_prover::SP1CudaProverBuilder;
    ...
    use sp1_sdk::setup_logger;

    use crate::{
        config::Felt,
        test_utils::tracegen_setup::{
            self, CORE_MAX_LOG_ROW_COUNT, CORE_MAX_TRACE_SIZE, FIBONACCI_ELF, LOG_STACKING_HEIGHT,
        },
    ...
    async fn test_poseidon2_koala_bear_16() {
        ...
        run_in_place(|scope| async move {
            let prover = SP1CudaProverBuilder::new(scope.clone())
                .normalize_cache_size(1)
                .set_max_compose_arity(4)
                .without_vk_verification()
                .build()
                .await;

            let (pk, _, _) = prover.core().setup(FIBONACCI_ELF).await;
            let pk = unsafe { pk.into_inner() };

            let merkle_prover = csl_merkle_tree::Poseidon2KoalaBear16CudaProver::default();

            let preprocessed_traces = pk.preprocessed_data.preprocessed_traces.clone();
            ...
            let (merkle_commitment, _) =
                merkle_prover.commit_tensors(interleaved_message).await.unwrap();
            ...
            assert_eq!(new_main_commit, merkle_commitment);
        })
        .await
        .await
        .unwrap();
