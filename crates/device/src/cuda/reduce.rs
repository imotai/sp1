...
use csl_sys::{
    reduce::{
        ...
        partial_inner_product_baby_bear_base_extension_kernel,
        partial_inner_product_baby_bear_extension_kernel, partial_inner_product_baby_bear_kernel,
    },
...
use crate::{
    args,
    mem::DeviceData,
    tensor::{Dimensions, InnerProductBackend, SumBackend, TensorViewMut},
    Tensor,
};
...
fn block_sum<T: DeviceData, const BLOCK_SIZE: usize, const INTIAL_STRIDE: usize>(
...
impl<T: DeviceData> SumBackend<T> for TaskScope
where
    TaskScope: DeviceSumKernel<T>,
{
    fn sum_tensor_dim(
        src: &Tensor<T, Self>,
        dst: crate::tensor::TensorViewMut<T, Self>,
        dim: usize,
    ) {
        ...
        if height <= BLOCK_SIZE {
            block_sum::<T, BLOCK_SIZE, INTIAL_STRIDE>(src, dst, dim);
            return;
        }
...
/// A kernel that computes inner product of two tensors along a given dimension along each block.
///
/// # Safety
pub unsafe trait DeviceInnerProductKernel<T, K>: DeviceSumKernel<T> {
    fn partial_inner_product_kernel() -> KernelPtr;
}

impl<T: DeviceData, K: DeviceData> InnerProductBackend<T, K> for TaskScope
where
    TaskScope: DeviceInnerProductKernel<T, K>,
{
    fn inner_product_tensor_dim(
        ...
        assert!(
            dim == src1.sizes().len() - 1,
            "only inner product over the last dimension is supported"
        );

        let height = src1.sizes()[dim];
        let width = src1.total_len() / height;

        let null_ptr = std::ptr::null::<c_void>();
        let partial_args = args!(null_ptr, src1.as_ptr(), src2.as_ptr(), width, height);
        const BLOCK_SIZE: usize = 512;
        const INTIAL_STRIDE: usize = 8;
        unsafe {
            partial_sum_reduction_into::<T, BLOCK_SIZE, INTIAL_STRIDE, 5>(
                dst,
                TaskScope::partial_inner_product_kernel(),
                partial_args,
                0,
                &src1.dimensions,
                dim,
...
unsafe impl DeviceInnerProductKernel<BabyBear, BabyBear> for TaskScope {
    fn partial_inner_product_kernel() -> KernelPtr {
        unsafe { partial_inner_product_baby_bear_kernel() }
    }
}

unsafe impl
    DeviceInnerProductKernel<
        BinomialExtensionField<BabyBear, 4>,
        BinomialExtensionField<BabyBear, 4>,
    > for TaskScope
{
    fn partial_inner_product_kernel() -> KernelPtr {
        unsafe { partial_inner_product_baby_bear_extension_kernel() }
    }
}

unsafe impl DeviceInnerProductKernel<BinomialExtensionField<BabyBear, 4>, BabyBear> for TaskScope {
    fn partial_inner_product_kernel() -> KernelPtr {
        unsafe { partial_inner_product_baby_bear_base_extension_kernel() }
    }
}
...
mod tests {
    ...
    async fn test_baby_bear_sum() {
        ...
        for size in [10, 100, 1 << 16] {
            let values: Vec<BabyBear> = (0..(num_summands * size)).map(|_| rng.gen()).collect();
            let tensor =
                HostTensor::<BabyBear>::from(values).reshape([num_summands, size]).unwrap();

            let tensor_sent = tensor.clone();
            let sum_tensor = crate::cuda::task()
                .await
                .unwrap()
                .run(|t| async move {
                    let device_tensor = DeviceTensor::from_host(tensor_sent, t).await.unwrap();
            ...
            for i in 0..num_summands {
                let expected_sum: BabyBear = tensor.index(i).as_slice().iter().copied().sum();
    ...
    async fn test_baby_bear_ext_sum() {
        ...
        let values: Vec<EF> = (0..(num_summands * size)).map(|_| rng.gen()).collect();
        let tensor = HostTensor::<EF>::from(values).reshape([num_summands, size]).unwrap();
        ...
        let sum_tensor = crate::cuda::task()
            ...
            .unwrap()
            .run(|t| async move {
                let device_tensor = DeviceTensor::from_host(tensor_sent, t.clone()).await.unwrap();
                let sums = device_tensor.sum(1);
                sums.into_host().await.unwrap()
            })
            .await
        ...
        assert_eq!(sum_tensor.sizes(), [num_summands]);
        for i in 0..num_summands {
            let expected_sum: EF = tensor.index(i).as_slice().iter().copied().sum();
            assert_eq!(expected_sum, *sum_tensor[[i]]);
        }
    }

    #[tokio::test]
    async fn test_baby_bear_inner_product() {
        let num_summands = 100;
        let mut rng = rand::thread_rng();

        for size in [10, 100, 1 << 16] {
            let values_1: Vec<BabyBear> = (0..(num_summands * size)).map(|_| rng.gen()).collect();
            let values_2: Vec<BabyBear> = (0..(num_summands * size)).map(|_| rng.gen()).collect();
            let tensor_1 =
                HostTensor::<BabyBear>::from(values_1).reshape([num_summands, size]).unwrap();
            let tensor_2 =
                HostTensor::<BabyBear>::from(values_2).reshape([num_summands, size]).unwrap();

            let tensor_sent_1 = tensor_1.clone();
            let tensor_sent_2 = tensor_2.clone();
            let inner_product = crate::cuda::task()
                .await
                .unwrap()
                .run(|t| async move {
                    let device_tensor_1 =
                        DeviceTensor::from_host(tensor_sent_1, t.clone()).await.unwrap();
                    let device_tensor_2 =
                        DeviceTensor::from_host(tensor_sent_2, t.clone()).await.unwrap();
                    let inner_product = device_tensor_1.inner_product(&device_tensor_2, 1);
            ...
            for i in 0..num_summands {
                let expected_inner_product: BabyBear = tensor_1
                    .index(i)
                    .as_slice()
                    .iter()
                    .copied()
                    .zip(tensor_2.index(i).as_slice().iter().copied())
                    .map(|(a, b)| a * b)
                    .sum();
    ...
    #[tokio::test]
    async fn test_baby_bear_ext_ext_inner_product() {
        let num_summands = 100;
        let mut rng = rand::thread_rng();
        ...
        for size in [10, 100, 1 << 16] {
            ...
            let values_2: Vec<EF> = (0..(num_summands * size)).map(|_| rng.gen()).collect();
            let tensor_1 = HostTensor::<EF>::from(values_1).reshape([num_summands, size]).unwrap();
            let tensor_2 = HostTensor::<EF>::from(values_2).reshape([num_summands, size]).unwrap();
            ...
            assert_eq!(inner_product.sizes(), [num_summands]);
            for i in 0..num_summands {
                let expected_inner_product: EF = tensor_1
                    .index(i)
                    .as_slice()
                    .iter()
                    .copied()
    ...
    #[tokio::test]
    async fn test_baby_bear_base_ext_inner_product() {
        let num_summands = 100;
        let mut rng = rand::thread_rng();
        ...
        for size in [10, 100, 1 << 16] {
            let values_1: Vec<EF> = (0..(num_summands * size)).map(|_| rng.gen()).collect();
            let values_2: Vec<F> = (0..(num_summands * size)).map(|_| rng.gen()).collect();
            let tensor_1 = HostTensor::<EF>::from(values_1).reshape([num_summands, size]).unwrap();
            let tensor_2 = HostTensor::<F>::from(values_2).reshape([num_summands, size]).unwrap();
