use super::CudaSend;
use crate::cuda::{stream::StreamRef, CudaError, CudaEvent, TaskScope};
...
mod tests {
    ...
    use crate::cuda::{buffer::DeviceBuffer, TaskPoolBuilder};
    ...
    async fn test_channel() {
        ...
        let (handle_1, handle_2) = tokio::join!(
            ...
            task_2.run(|t| async move {
                ...
                assert_eq!(buf.to_vec().await.unwrap(), vec![1; 1000000]);
            }),
        );
    ...
    async fn test_owned_channel() {
        let task_1 = crate::cuda::owned_task().await.unwrap();
        let task_2 = crate::cuda::owned_task().await.unwrap();
