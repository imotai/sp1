...
pub mod test_fixtures {
    ...
    use sp1_recursion_executor::{
        instruction, linear_program, Address, BaseAluInstr, BaseAluIo, BaseAluOpcode,
        BatchFRIBaseVecIo, BatchFRIEvent, BatchFRIExtSingleIo, BatchFRIExtVecIo, BatchFRIInstr,
        Block, CommitPublicValuesEvent, ExecutionRecord, ExpReverseBitsEvent, ExpReverseBitsInstr,
        ExpReverseBitsIo, ExtAluInstr, ExtAluIo, ExtAluOpcode, FriFoldBaseIo, FriFoldEvent,
        FriFoldExtSingleIo, FriFoldExtVecIo, FriFoldInstr, Instruction, Poseidon2Event,
        Poseidon2Instr, Poseidon2Io, RecursionProgram, RecursionPublicValues, SelectInstr,
        SelectIo, RECURSIVE_PROOF_NUM_PV_ELTS,
    };
    ...
    fn public_values_events() -> Vec<CommitPublicValuesEvent<BabyBear>> {
        ...
        let mut events = Vec::with_capacity(num_test_cases);
        for _ in 0..num_test_cases {
            let random_felts: [BabyBear; RECURSIVE_PROOF_NUM_PV_ELTS] =
    ...
    fn public_values_instructions() -> Vec<Instruction<BabyBear>> {
        ...
        for _ in 0..num_test_cases {
            ...
            instructions.push(instruction::commit_public_values(public_values));
        }
        instructions
    }
