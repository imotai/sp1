...
use sp1_recursion_executor::{
    Address, BatchFRIEvent, BatchFRIInstr, Block, ExecutionRecord, Instruction, RecursionProgram,
};
...
impl<F: PrimeField32, const DEGREE: usize> MachineAir<F> for BatchFRIChip<DEGREE> {
    ...
    fn generate_preprocessed_trace(&self, program: &Self::Program) -> Option<RowMajorMatrix<F>> {
        ...
        pad_rows_fixed(&mut rows, || [BabyBear::zero(); NUM_BATCH_FRI_PREPROCESSED_COLS], None);
...
mod tests {
    ...
    fn generate_preprocessed_trace_reference<const DEGREE: usize>(
        ...
        pad_rows_fixed(&mut rows, || [F::zero(); NUM_BATCH_FRI_PREPROCESSED_COLS], None);
