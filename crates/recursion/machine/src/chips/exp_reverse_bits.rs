...
use sp1_recursion_executor::{
    ExecutionRecord, ExpReverseBitsEvent, ExpReverseBitsInstr, Instruction, RecursionProgram,
};
...
mod tests {
    ...
    use sp1_recursion_executor::{
        instruction as instr, linear_program, Address, ExpReverseBitsIo, MemAccessKind,
    };
    ...
    #[tokio::test]
    async fn prove_babybear_circuit_erbl() {
        setup_logger();
        type F = BabyBear;
    ...
    fn generate_preprocessed_trace_reference(
        ...
        pad_rows_fixed(&mut rows, || [F::zero(); NUM_EXP_REVERSE_BITS_LEN_PREPROCESSED_COLS], None);
