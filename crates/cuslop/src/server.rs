...
impl Server {
    ...
    pub async fn run(self, task_scope: TaskScope) {
        ...
        // If something bad happens, lets just forcibly destroy the orphaned socket.
        ...
        let prover = SP1CudaProverBuilder::new(task_scope).build().await;
    ...
    pub async fn prove_core(&mut self, id: [u8; 32], stdin: SP1Stdin) -> Response {
        ...
        let Some(cached) = self.pk_cache.get(&id) else {
            return Response::InternalError(
                "Missing key for core proof, this is a bug.".to_string(),
            );
        };
