...
/// A set of columns to read operations with op_a being a register and op_c being an
/// immediate. `op_b` contains an immediate in bits 63:16 and a register ID in bits 15:0.
///
/// This is like the `ITypeReader`, but we read `op_b` into a `Word<T>` instead of a `T`.
#[derive(AlignedBorrow, Default, Debug, Clone, Copy)]
#[repr(C)]
pub struct JalrTypeReader<T> {
    pub op_a: T,
...
impl<F: PrimeField32> JalrTypeReader<F> {
    pub fn populate(
        &mut self,
...
impl<F: Field> JalrTypeReader<F> {
    ...
    pub fn eval<AB: SP1CoreAirBuilder>(
        ...
        builder.eval_memory_access_in_shard_read(
            ...
            [cols.op_b[0].into(), AB::Expr::zero(), AB::Expr::zero()],
            cols.op_b_memory,
            is_real,
        );
    }
