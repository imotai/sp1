...
mod tests {
    ...
    use sp1_stark::{
        ...
        Chip, CpuProver, StarkMachine,
    };
    use test_artifacts::U256XU2048_MUL_ELF;

    use crate::{
        io::SP1Stdin,
        utils::{self, run_test, run_test_machine, setup_test_machine},
    };
    use crate::{
    ...
    fn test_u256x2048_mul_pass() {
        ...
        let air = U256x2048MulChip::new();
        <U256x2048MulChip as MachineAir<BabyBear>>::generate_trace(
            &air,
            &execution_record,
        ...
        // Run the test.
        let air = U256x2048MulChip::new();
        let chip: Chip<BabyBear, U256x2048MulChip> = Chip::new(air);
        let machine = StarkMachine::new(config.clone(), vec![chip], SP1_PROOF_NUM_PV_ELTS, true);
        run_test_machine::<BabyBearPoseidon2, U256x2048MulChip>(
            vec![execution_record],
            machine,
            pk,
            vk,
        )
