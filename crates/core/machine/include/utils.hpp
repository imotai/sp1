...
write_word_from_u32(Word<decltype(F::val)>& word, const uint32_t value) {
    word._0[0] = F::from_canonical_u16(value).val;
    word._0[1] = F::from_canonical_u16(value >> 16).val;
}

template<class F>
__SP1_HOSTDEV__ __SP1_INLINE__ void
write_word_from_u32_v2(Word<F>& word, const uint32_t value) {
    word._0[0] = F::from_canonical_u16(value);
    word._0[1] = F::from_canonical_u16(value >> 16);
}
...
word_to_u32(const Word<decltype(F::val)>& word) {
       return ((uint32_t)F(word._0[0]).as_canonical_u32())
        + ((uint32_t)F(word._0[1]).as_canonical_u32() << 16);
}
...
__SP1_HOSTDEV__ __SP1_INLINE__ void word_from_le_bytes(
    ...
    // Combine bytes[0,1] for first limb and bytes[2,3] for second limb
    word._0[0] = F::from_canonical_u16((uint16_t)bytes[0] | ((uint16_t)bytes[1] << 8)).val;
    word._0[1] = F::from_canonical_u16((uint16_t)bytes[2] | ((uint16_t)bytes[3] << 8)).val;
}
