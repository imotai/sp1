...
pub(crate) unsafe fn write(ctx: &mut JitContext, arg1: u64, arg2: u64) -> Option<u64> {
    ...
    let bytes = (0..nbytes).map(|i| memory.byte(write_buf + i)).collect::<Vec<u8>>();
    ...
    let hook_return = match fd as u32 {
        ...
        FD_FP_SQRT => Some(fp_ops::hook_fp_sqrt(slice)),
        FD_ECRECOVER_HOOK => Some(hook_ecrecover(slice)),
        FD_EDDECOMPRESS => Some(hook_ed_decompress(slice)),
        FD_RSA_MUL_MOD => Some(hook_rsa_mul_mod(slice)),
        _ => {
...
#[must_use]
pub fn hook_ed_decompress(buf: &[u8]) -> Vec<Vec<u8>> {
...
#[must_use]
pub fn hook_rsa_mul_mod(buf: &[u8]) -> Vec<Vec<u8>> {
    assert_eq!(
        buf.len(),
        256 + 256 + 256,
