...
impl MinimalExecutor {
    ...
    fn transpile_instructions<B: SP1RiscvTranspiler>(&mut self, mut backend: B) {
        ...
        for (i, instruction) in self.program.instructions.iter().enumerate() {
            ...
            let next_pc = ((i + 1) * 4) as u32 + self.program.pc_base;
            ...
            if !(instruction.is_branch_instruction()
                ...
                || instruction.is_auipc_instruction()
    ...
    fn transpile_alu_instruction<B: SP1RiscvTranspiler>(
        ...
        match instruction.opcode {
            Opcode::ADD | Opcode::ADDI => backend.risc_add(b, c, rd),
            Opcode::SUB => backend.risc_sub(b, c, rd),
            Opcode::XOR => backend.risc_xor(b, c, rd),
            Opcode::OR => backend.risc_or(b, c, rd),
            Opcode::AND => backend.risc_and(b, c, rd),
            Opcode::SLL => backend.risc_sll(b, c, rd),
            Opcode::SRL => backend.risc_srl(b, c, rd),
            Opcode::SRA => backend.risc_sra(b, c, rd),
            Opcode::SLT => backend.risc_slt(b, c, rd),
            Opcode::SLTU => backend.risc_sltu(b, c, rd),
            Opcode::MUL => backend.risc_mul(b, c, rd),
            Opcode::MULH => backend.risc_mulh(b, c, rd),
            Opcode::MULHU => backend.risc_mulhu(b, c, rd),
            Opcode::MULHSU => backend.risc_mulhsu(b, c, rd),
            Opcode::DIV => backend.risc_div(b, c, rd),
            Opcode::DIVU => backend.risc_divu(b, c, rd),
            Opcode::REM => backend.risc_rem(b, c, rd),
            Opcode::REMU => backend.risc_remu(b, c, rd),
...
mod test {
    ...
    fn run_program_and_compare_end_state(program: Elf) {
        ...
        eprintln!("interpreter mhz={} mhz", interpreter_mhz);
